...
const PROVIDERS = [
  {
    id: "openclaw",
    name: "OpenClaw",
    description: "Recommended Free tier",
    baseUrl: "https://api.openclaw.com/v1",
    models: ["gpt-4o-mini", "claude-sonnet-4-20250514", "deepseek-chat"],
    icon: "🦞",
  },
  {
    id: "anthropic",
...
export function ApiSettingsDrawer({ open, onOpenChange }: ApiSettingsDrawerProps) {
  ...
  async function handleTestConnection() {
    ...
    try {
      ...
      console.log("[Settings] 🔍 Testing connection with full request body:", JSON.stringify(requestBody, null, 2))
  ...
  function handleSwitchProvider(providerId: string) {
    ...
    if (provider) {
      ...
      console.log("[Settings] Switched to:", provider.id, "API Key preserved:", Boolean(config.apiKey))
    }
  }
        ...
        <div className="p-6 space-y-6">
              ...
              {PROVIDERS.map((provider) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-muted-foreground font-mono mt-0.5 truncate">{provider.baseUrl}</p>
                  </div>
          ...
          <div className="space-y-2">
            ...
            {config.provider && (
              <p className="text-[10px] text-muted-foreground">
                ...
                {config.provider === "anthropic" && "Get your key from https://console.anthropic.com"}
                {config.provider === "openai" && "Get your key from https://platform.openai.com/api-keys"}
                {config.provider === "google" && "Get your key from https://aistudio.google.com/app/apikey"}
                {config.provider === "deepseek" && "Get your key from https://platform.deepseek.com"}
          ...
          <div className="space-y-4 pt-4 border-t border-border">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
                ...
                <span className="font-mono text-muted-foreground">{config.temperature.toFixed(1)}</span>
              </div>
              <input
                type="range"
                min="0"
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={(e) => setConfig({ maxTokens: parseInt(e.target.value) || 4000 })}
                className="w-full px-3 py-2 bg-background border border-border rounded-md text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary"
                min="100"
                max="32000"
              />
            ...
            <div className="p-3 rounded-md bg-secondary/30 text-xs font-mono space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span className={hasKey ? "text-success font-medium" : "text-danger"}>
                  {hasKey ? `✓ ${config.apiKey.slice(0, 8)}...${config.apiKey.slice(-4)}` : "✗ Not set"}
                </span>
              </div>
            </div>
          ...
          <div className="pt-4 border-t border-border text-[10px] text-muted-foreground space-y-1">
            ...
            <div className="flex items-center gap-1">
              ...
              <a href="#" className="underline hover:no-underline">Documentation</a>
              <span>·</span>
