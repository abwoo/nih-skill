...
async function searchPubMed(query: string, limit: number) {
  try {
    ...
    for (
      var batchStart = 0;
      batchStart < idList.length && allResults.length < limit;
      batchStart += 10
    ) {
...
async function searchOpenAlex(query: string, limit: number) {
  try {
    ...
    var params = new URLSearchParams({
      ...
      select:
        'id,title,authorships,publication_year,doi,pmid,primary_location,abstract_inverted_index,cited_by_count,type,open_access',
...
async function searchCrossref(query: string, limit: number) {
  try {
    ...
    var url =
      ...
      '&rows=' +
      limit * 2 +
      '&sort=relevance&order=desc&select=DOI,title,author,container-title,created-date,ISBN,ISSN,type,published-online,URL,is-referenced-by-count';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    var allResults: RawPaperResult[] = [];
    var apiStatus: Record<string, SearchAPIStatus> = {};
    ...
    if (source === 'pubmed' || source === 'all') {
      promises.push(
        searchPubMed(query, Math.ceil(limit * 1.5))
          .then(function (res) {
    ...
    if (source === 'arxiv') {
      promises.push(
        searchArXiv(query, Math.ceil(limit * 1.5))
          .then(function (res) {
    ...
    if (source === 'crossref') {
      promises.push(
        searchCrossref(query, Math.ceil(limit * 1.5))
          .then(function (res) {
      ...
      if (result.status === 'fulfilled') {
        ...
        if (err) {
          ...
          for (var j = 0; j < res.length; j++) {
            ...
            var isDuplicate = allResults.some(function (r: RawPaperResult) {
              if (r.doi && item.doi && r.doi === item.doi) return true;
              if (
                r.arxivId &&
                (item as RawPaperResult).arxivId &&
                r.arxivId === (item as RawPaperResult).arxivId
              )
                return true;
    ...
    logSearch(
      'MAIN',
      'Final: ' + finalResults.length + '/' + allResults.length + ' | Time: ' + elapsedTime + 'ms'
    );
...
- Return exactly ${limit} results or fewer if not enough relevant papers exist`;
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a helpful academic search assistant. Always respond with valid JSON arrays of paper information.',
          },
          {
            role: 'user',
  ...
  try {
    ...
    // 验证并清理数据
    papers = papers
      .filter((p: RawPaperResult) => p && p.title)
      .map(
        (p: RawPaperResult): RawPaperResult => ({
          title: p.title || '',
          authors: p.authors || '',
          year: p.year || new Date().getFullYear(),
          doi: p.doi || '',
          abstract: p.abstract || '',
          source: (p.source || 'LLM-Knowledge') as DatabaseSource,
    ...
    logSearch(
      'LLM-Search',
      `Failed to parse LLM response as JSON: ${parseError instanceof Error ? parseError.message : parseError}`
    );
    // 如果JSON解析失败，返回空数组
    papers = [];
  }
...
function formatAndReturnResults(
  ...
  if (yearFrom)
    filteredResults = filteredResults.filter(function (r) {
      return !r.year || r.year >= yearFrom;
    });
  if (yearTo)
    filteredResults = filteredResults.filter(function (r) {
      return !r.year || r.year <= yearTo;
    });
  ...
  return NextResponse.json({
    ...
    apiStatus: apiStatus,
    performance: { elapsedTimeMs: elapsedTime },
    results: finalResults.map(function (paper: RawPaperResult): PaperResult {
      return {
        title: paper.title,
        authors: paper.authors,
        year: paper.year,
        ...
        abstract: paper.abstract || '',
        url: paper.url,
        pdfUrl: (paper as Record<string, unknown>).pdfUrl as string | undefined,
        source: paper.source,
        citationCount: paper.citationCount,
        venue: paper.venue,
      };
