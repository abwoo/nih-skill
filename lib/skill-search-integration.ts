...
export async function searchLiteratureWithContext(
  ...
  try {
    ...
    const hasVerifiedResults = searchResult.verificationStatus.overall === 'VERIFIED' ||
...
function generateUserSummary(result: EnhancedSearchResult, context: SearchContext): string {
  ...
  if (verificationStatus) {
    const emoji = verificationStatus.overall === 'VERIFIED' ? '✅' :
                  verificationStatus.overall === 'PARTIAL' ? '⚠️' : '⚡'
  ...
  if (optimizedQuery && optimizedQuery !== context.userQuery && optimizedQuery.length > context.userQuery.length * 1.5) {
    summary += `\n💡 *系统自动优化了您的查询以获得更好的结果*`
  }
