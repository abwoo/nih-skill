...
export class SkillFacade {
  ...
  async execute(request: UnifiedSkillRequest): Promise<UnifiedSkillResponse> {
    ...
    try {
      ...
      return this.formatUnifiedResponse(
        result,
        {
          ...request,
          intent: detectedIntent || 'analyze',
  ...
  private formatUnifiedResponse(
    ...
    return {
        ...
        request.depth !== 'quick'
          ? {
              ...
              innovationLevel: {
                ...
                label:
                  INNOVATION_LEVELS[
                    rawResult.output?.body?.innovation_level as keyof typeof INNOVATION_LEVELS
              ...
              fatalBlockers: {
                ...
                criticalIssues: FATAL_BLOCKERS.filter((b: any) => b.severity_levels?.includes('🔴'))
                  .length,
                warningIssues: FATAL_BLOCKERS.filter((b: any) => b.severity_levels?.includes('🟡'))
                  .length,
              ...
              clinicalValidation: {
                ...
                label:
                  (CLINICAL_VALIDATION_LEVELS as any)[
                    rawResult.output?.body?.clinical_validation_level
      ...
      recommendations: {
        ...
        referencesToConsult: (
          Array.from(rawResult.context?.loadedReferences?.keys() || []) as string[]
        ).slice(0, 4),
      },
    ...
    return {
      phases: [
        ...
        },
        {
          ...
          priority: depth !== 'quick' ? 'mandatory' : 'recommended',
          steps: [
            'Paradigm cluster identification',
            'Gap analysis between clusters',
            'Frontier prediction',
          ],
          tools: [],
...
export async function comparePapers(dois: string[]): Promise<UnifiedSkillResponse> {
  return getSkillFacade().compare(
    dois.map((doi) => ({ type: 'doi' as const, content: doi })) as any
  );
}
