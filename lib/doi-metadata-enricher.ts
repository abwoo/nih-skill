...
async function fetchCrossrefMetadata(doi: string, timeout: number): Promise<Partial<EnhancedPaperMetadata>> {
  try {
...
async function fetchSemanticScholarMetadata(doi: string, timeout: number): Promise<Partial<EnhancedPaperMetadata>> {
  try {
    const fields = [
      "title",
      "authors",
    ...
    return {
      ...
      year: data.year,
      journal: data.venue || data.publicationVenue?.name || "",
      ...
      semanticScholar: {
        ...
        referencesCount: data.referenceCount || 0,
        openAccess: data.isOpenAccess || false,
      ...
      ...(data.externalIds?.Pubmed && {
        pubmed: {
          pmid: String(data.externalIds.Pubmed),
          pmcid: data.externalIds.PubmedCentral ? String(data.externalIds.PubmedCentral) : "",
          meshTerms: [],
          grantList: [],
          publicationTypes: [],
        },
    ...
    console.error("[enrichment] Semantic Scholar error:", error)
    throw error
  }
}

async function fetchOpenAlexMetadata(doi: string, timeout: number): Promise<Partial<EnhancedPaperMetadata>> {
  try {
    ...
    return {
      ...
      openAlex: {
        concepts:
          data.concepts?.map((c: { id: string; display_name: string; score: number; level: number }) => ({
            id: c.id,
...
async function fetchUnpaywallData(doi: string, timeout: number): Promise<Partial<EnhancedPaperMetadata>> {
  try {
...
async function fetchPubMedMetadata(pmid: string, timeout: number): Promise<Partial<EnhancedPaperMetadata>> {
  try {
    ...
    return {
      pubmed: {
        ...
        meshTerms:
          article.meshheadinglist?.meshheading?.map(
            (m: { descriptorname: { string: string[] } }) =>
...
function mergeMetadata(
  ...
  for (const source of sources) {
    ...
    if (source.semanticScholar && !merged.semanticScholar) {
      merged.semanticScholar = source.semanticScholar
    } else if (source.semanticScholar && merged.semanticScholar) {
      Object.assign(merged.semanticScholar, source.semanticScholar)
    }
...
export async function enrichDOIMetadata(
  ...
  try {
      ...
      try {
        ...
        switch (source) {
          ...
          case "semantic_scholar":
            if (!fullConfig.enableSemanticScholar) continue
            metadata = await fetchSemanticScholarMetadata(extractedDOI.doi, fullConfig.timeout.semanticScholar)
            break
    ...
    if (fullConfig.enableUnpaywall) {
      try {
        const unpaywallData = await fetchUnpaywallData(extractedDOI.doi, fullConfig.timeout.unpaywall)
        if (unpaywallData) {
    ...
    if (finalMetadata.pubmed?.pmid && fullConfig.enablePubMed) {
      try {
        const pubmedData = await fetchPubMedMetadata(finalMetadata.pubmed.pmid, fullConfig.timeout.pubMed)
        if (pubmedData) {
