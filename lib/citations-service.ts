...
async function getOpenAlexCitations(
  ...
  const paperRes = await fetch(doiUrl, {
  ...
  if (direction === 'citations') {
    ...
    const res = await fetch(citedByUrl, {
      headers: { 'User-Agent': 'BioMedResearchAgent/1.0 (mailto:agent@biomed.dev)' },
    });
    ...
    return {
      results: (data.results || []).map((w: Record<string, unknown>) => ({
        title: w.title || '',
        authors:
          (w.authorships as Array<Record<string, unknown>>)
        ...
        venue:
          ((w.primary_location as Record<string, unknown>)?.source as Record<string, string>)
            ?.display_name || '',
      })),
      paperTitle,
    };
