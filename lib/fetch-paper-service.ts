...
export function detectSourceType(
  input: string
): 'doi' | 'arxiv' | 'pmcid' | 'pmid' | 'url' | 'unknown' {
    ...
    try {
      ...
      if (res.status === 429 && attempt < maxRetries) {
        ...
        const delay = retryAfter
          ...
          : Math.min(1000 * (attempt + 1) * 2, 30000);
        console.warn(
          `[fetch-paper] Rate limited (${res.status}), retrying in ${delay}ms... (attempt ${attempt + 1}/${maxRetries})`
        );
      ...
      if (res.status >= 500 && attempt < maxRetries) {
        ...
        console.warn(
          `[fetch-paper] Server error (${res.status}), retrying in ${delay}ms... (attempt ${attempt + 1}/${maxRetries})`
        );
        await new Promise((resolve) => setTimeout(resolve, delay));
        continue;
      }
      ...
      if (attempt < maxRetries && lastError.name !== 'AbortError') {
        const delay = 1000 * (attempt + 1);
        console.warn(
          `[fetch-paper] Request failed: ${lastError.message}, retrying in ${delay}ms... (attempt ${attempt + 1}/${maxRetries})`
        );
        await new Promise((resolve) => setTimeout(resolve, delay));
...
async function fetchByDoi(doi: string): Promise<FetchResult> {
  ...
  try {
    ...
    const crossRefRes = await fetchWithRetry(
      crossRefUrl,
      {
    ...
    try {
      ...
      if (unpaywallRes.ok) {
        ...
        if (bestOa?.url_for_pdf) {
          try {
                ...
                if (pdfBuffer.length > 500) {
                  ...
                  if (pdfData.text?.trim()) {
                    fullText += `## Full Text (Open Access PDF)\n${pdfData.text
                      ...
                      .trim()}\n\n`;
                    console.log(
                      `[fetch-paper] Successfully fetched PDF via Unpaywall for DOI: ${cleanDoi}`
                    );
                  }
                }
              }
            ...
            console.warn(
              `[fetch-paper] PDF fetch failed for DOI ${cleanDoi}:`,
      ...
      console.warn(
        `[fetch-paper] Unpaywall lookup failed for DOI ${cleanDoi}:`,
        upwErr instanceof Error ? upwErr.message : String(upwErr)
      );
    }
    ...
    console.log(
      `[fetch-paper] Success - DOI: ${cleanDoi}, Title: "${title}", Has PDF: ${fullText.includes('Full Text')}`
    );
...
async function fetchByArxiv(arxivId: string): Promise<FetchResult> {
  ...
  try {
    ...
    const pdfLink =
      xml.match(/href="([^"]+pdf[^"]*)"/)?.[1] || `https://arxiv.org/pdf/${cleanId}.pdf`;
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        console.warn(
          `[fetch-paper] PDF fetch attempt ${attempt + 1} failed for arXiv:`,
          pdfErr instanceof Error ? pdfErr.message : String(pdfErr)
        );
        if (attempt === 0) {
    ...
    if (!pdfFetched) {
      console.warn(
        `[fetch-paper] Could not fetch PDF for arXiv: ${cleanId}, returning metadata only`
      );
    }
...
async function fetchByUrl(url: string): Promise<FetchResult> {
  ...
  try {
      ...
      try {
        ...
        if (!text) {
          return {
            ...
            suggestion:
              'This may be a scanned/image-based PDF. Try using a different source or DOI.',
          };
        }
    ...
    if (contentType.includes('html') || contentType.includes('text')) {
      ...
      if (!text || text.length < 50) {
        return {
          ...
          suggestion:
            'The page may require JavaScript rendering or may be blocked. Try copying the content manually.',
        };
      }
    ...
    return {
      ...
      suggestion:
        'Check the URL is correct and accessible. The site may be down or blocking requests.',
    };
  }
}
